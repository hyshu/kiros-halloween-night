...
class DescriptiveTextSystem {
  ...
  void describeStateChange(String changeType, {Map<String, dynamic>? context}) {
    String description;

    switch (changeType) {
      case 'health_low':
        description =
            'You feel weakened. Perhaps some candy could restore your strength.';
        break;
      case 'inventory_full':
        description =
            'Your pockets are full of candy. Consider using some to make allies.';
        break;
      case 'no_allies':
        description =
            'You journey alone through the haunted realm. Allies could provide valuable assistance.';
        break;
      case 'many_allies':
        final count = context?['ally_count'] ?? 0;
        description =
            'You are accompanied by $count loyal allies. Their presence gives you confidence.';
        break;
      case 'area_cleared':
        description =
            'This area seems peaceful now. The hostile presence has lifted.';
        break;
      case 'danger_nearby':
        description =
      ...
      case 'safe_area':
        description =
            'This area feels safe and welcoming. A good place to rest and plan your next move.';
        break;
      default:
  ...
  void describeSpecialEvent(String eventType, {Map<String, dynamic>? context}) {
    String description;
    ...
    switch (eventType) {
      case 'secret_area_found':
        ...
        description =
            'You have discovered $areaName! This secret area may contain valuable treasures.';
        break;
      case 'rare_candy_found':
        final candyName = context?['candy_name'] ?? 'a rare candy';
        description =
            'Incredible! You have found $candyName, a legendary confection with powerful properties.';
        break;
      case 'ancient_relic':
        final relicName = context?['relic_name'] ?? 'an ancient artifact';
        description =
            'You sense the presence of $relicName. Its power resonates through the realm.';
        break;
      case 'ghostly_whispers':
        description =
            'Ethereal whispers echo through the air, speaking of ancient secrets and forgotten lore.';
        break;
      case 'magical_aura':
        description =
            'A magical aura permeates this area, making the very air shimmer with otherworldly energy.';
        break;
      case 'time_distortion':
        description =
            'Time seems to flow differently here. Past and present blur together in mysterious ways.';
        break;
      default:
  ...
  void describeEmotionalMoment(
    ...
    switch (momentType) {
      case 'first_friend':
        description =
            'A warm feeling fills your ghostly heart. You are no longer alone in this realm.';
        break;
      case 'ally_sacrifice':
        final allyName = context?['ally_name'] ?? 'your ally';
        description =
            '$allyName\'s sacrifice will not be forgotten. Their loyalty has touched your soul.';
        break;
      case 'redemption':
        description =
            'Through kindness and compassion, you have brought light to the darkness.';
        break;
      case 'courage_found':
        description =
            'You feel a surge of courage. The challenges ahead no longer seem insurmountable.';
        break;
      case 'wisdom_gained':
        description =
            'Your experiences have taught you valuable lessons about friendship and perseverance.';
        break;
      case 'hope_restored':
        description =
            'Hope blooms in your heart like a flower in the darkness. The future looks brighter.';
        break;
      default:
  ...
  String _generateEnvironmentDescription(
    ...
    if (position.x < 50 && position.z < 100) {
      descriptions.add(
        'You find yourself in the entrance halls of the haunted realm.',
      );
    } else if (position.x > 150 && position.z > 300) {
      descriptions.add(
        'You approach the deeper, more dangerous regions of the realm.',
      );
    } else {
      descriptions.add(
        'You traverse the mysterious corridors of the haunted realm.',
      );
    }
    ...
    if (hasEnemies && hasCandy) {
      ...
      descriptions.add(
        'Sweet treasures await discovery in this peaceful area.',
      );
    }
  ...
  String _generateMovementDescription(Position from, Position to) {
    ...
    final index =
        (from.x + from.z + to.x + to.z) % atmosphericDescriptions.length;
