...
import 'dart:math';
import 'package:kiro_halloween_game/core/candy_spawner.dart';
...
void main() {
  group('CandySpawner', () {
    ...
    test('should respect minimum distance between candy', () {
      ...
      for (int i = 0; i < spawnedCandy.length; i++) {
        for (int j = i + 1; j < spawnedCandy.length; j++) {
          ...
          expect(
            distance,
            greaterThanOrEqualTo(spawner.minDistanceBetweenCandy),
          );
        }
      }
    });
    ...
    test('should create candy with unique IDs', () {
      ...
      expect(
        ids.length,
        equals(spawnedCandy.length),
      ); // All IDs should be unique
    });

    test('should spawn candy at specific positions', () {
      final positions = [Position(10, 10), Position(12, 12), Position(8, 8)];
    ...
    test('should create weighted random candy', () {
      ...
      final candyBarCount = candyList
          .where((c) => c.name == 'Candy Bar')
          .length;
      final gingerBreadCount = candyList
          .where((c) => c.name == 'Gingerbread')
          .length;
    ...
    test('should reset ID counter', () {
      spawner.createRandomCandyAt(Position(5, 5));
    ...
    test('should handle map with no valid spawn locations', () {
      ...
      for (int x = 0; x < TileMap.worldWidth; x++) {
        for (int z = 0; z < TileMap.worldHeight; z++) {
          if (x == 0 ||
              x == TileMap.worldWidth - 1 ||
              z == 0 ||
              z == TileMap.worldHeight - 1) {
