...
import '../../lib/core/proximity_detector.dart';
import '../../lib/core/spatial_index.dart';
import '../../lib/core/activation_manager.dart';
import '../../lib/core/proximity_system.dart';
...
void main() {
  group('ProximityDetector', () {
    ...
    test('identifies enemies in activation range', () {
      final inRange = proximityDetector.getEnemiesInActivationRange(player, enemies);
    ...
    test('identifies enemies outside activation range', () {
      final outOfRange = proximityDetector.getEnemiesOutsideActivationRange(player, enemies);
    ...
    test('finds closest enemy', () {
      final closest = proximityDetector.getClosestEnemyToPlayer(player, enemies);
    ...
    test('detects immediate threats', () {
      ...
      final hasThreat = proximityDetector.hasImmediateThreat(player, threateningEnemies);
      
      expect(hasThreat, isTrue);
    });

    test('calculates proximity priority correctly', () {
      final prioritized = proximityDetector.getEnemiesByProximityPriority(player, enemies);
    ...
    test('determines activation status correctly', () {
      expect(proximityDetector.shouldActivateEnemy(player, enemies[0]), isTrue); // Within range
      expect(proximityDetector.shouldActivateEnemy(player, enemies[1]), isFalse); // Out of range
    });
  });
  ...
  group('SpatialIndex', () {
    ...
    setUp(() {
      spatialIndex = SpatialIndex(worldWidth: 100, worldHeight: 100, cellSize: 10);
    ...
    test('adds and retrieves characters correctly', () {
      final charactersInRadius = spatialIndex.getCharactersInRadius<EnemyCharacter>(
        Position(15, 15), 
        5,
      );
  ...
  group('ActivationManager', () {
    ...
    setUp(() {
      ...
      enemies = List.generate(20, (i) => EnemyCharacter.human(
        id: 'enemy$i',
  ...
  group('ProximitySystem Integration', () {
    ...
    test('integrates all components correctly', () {
      // Add enemies
      final enemies = List.generate(10, (i) => EnemyCharacter.human(
        id: 'enemy$i',
