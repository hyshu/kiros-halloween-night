...
import '../../lib/core/enemy_manager.dart';
...
void main() {
  group('Enemy Integration Tests', () {
    test('should spawn enemies in generated world', () async {
      ...
      final spawnedEnemies = enemyManager.spawnEnemiesForTesting(
      ...
      for (final enemy in enemyManager.enemies.values) {
        expect(tileMap.isWalkable(enemy.position), true,
            reason: 'Enemy ${enemy.id} at ${enemy.position} should be on walkable tile');
      }
      
      // Verify enemies have proper spacing
      final enemyPositions = enemyManager.enemies.values.map((e) => e.position).toList();
    ...
    test('should activate enemies based on player proximity', () async {
      ...
      for (final enemy in activeEnemies) {
        ...
        expect(distance, lessThanOrEqualTo(enemy.activationRadius),
    ...
    test('should provide enemy statistics', () async {
      ...
      final totalByType = (stats['enemy_types'] as Map<String, int>)
          .values.fold(0, (sum, count) => sum + count);
