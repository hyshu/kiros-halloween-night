...
void main() {
  group('TileMap Integration Tests', () {
    test('should work with existing Position class methods', () {
      ...
      // Test Position methods work with TileMap
      const center = Position(250, 500);
      const adjacent = Position(251, 500);
      ...
      expect(worldX, equals(500.0)); // 250 * 2.0
      expect(worldY, equals(0.0));
      expect(worldZ, equals(1000.0)); // 500 * 2.0
    });
    ...
    test('should handle large world dimensions correctly', () {
      ...
      // Test corners of the large world
      ...
      // Test that we can set tiles across the large world
      const farPosition = Position(450, 950);
      ...
      const start = Position(50, 50);
      const end = Position(450, 950);
      expect(start.distanceTo(end), equals(1300)); // Manhattan distance
    });
    ...
    test('should maintain boundary integrity across all operations', () {
      ...
      const topEdge = Position(250, 0);
      const bottomEdge = Position(250, 999);
      const leftEdge = Position(0, 500);
      const rightEdge = Position(499, 500);
      
      expect(() => tileMap.setTileAt(topEdge, TileType.floor), throwsArgumentError);
      expect(() => tileMap.setTileAt(bottomEdge, TileType.floor), throwsArgumentError);
      expect(() => tileMap.setTileAt(leftEdge, TileType.floor), throwsArgumentError);
      expect(() => tileMap.setTileAt(rightEdge, TileType.floor), throwsArgumentError);
    ...
    test('should support boss and spawn location management', () {
      ...
      // Test setting boss location
      const bossLocation = Position(400, 900);
